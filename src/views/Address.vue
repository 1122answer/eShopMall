        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListFilter" :class="{'check':checkIndex == index}" @click="checkIndex = index;selAddressId = item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':selAddressId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
    <model v-bind:mdShow="isMdShow" v-on:close="closeModal">
      <p slot="message">
        你确认要删除的此条数据
      </p>
      <div slot="btnGroup">
        <a class="btn btn-m" @click="delAddress">确认</a>
        <a class="btn btn-m" @click="isMdShow = false">关闭</a>
      </div>
    </model> 
      ...
      methods:{
        init(){
          axios.get("users/addressList").then((response)=>{
             ...
             this.selAddressId = this.addressList[0].addressId
          })
        },
        expand(){
          if (this.limit == 3) {
